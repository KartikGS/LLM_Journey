...
4. **Technical Sanity Check**: BA consults `$LLM_JOURNEY_ARCHITECTURE`, `$LLM_JOURNEY_TECHNICAL_CONTEXT`, and ADRs in `$LLM_JOURNEY_GOVERNANCE_DECISIONS` to identify potential conflicts or opportunities for "Product Shaping" (e.g., suggesting a fallback UI for a known browser constraint).
5. **Testing Incident Check (Conditional)**: If the task involves failing tests/lint/build or environment-specific runtime mismatches, BA must load `$LLM_JOURNEY_TESTING` and collect at least one command-based baseline (`exact command + result`) before finalizing CR scope.
6. BA creates structured requirement document.
7. BA assesses business complexity.
8. **Output:** `/LLM_Journey/a-docs/workflow/requirements/CR-XXX-<slug>.md` + prompt for Tech Lead.
...
4. **MANDATORY OUTPUT:** Tech Lead creates `/LLM_Journey/a-docs/workflow/plans/CR-XXX-plan.md` using the Standard Plan Template defined in `$LLM_JOURNEY_WORKFLOW_PLANS`.
...
6. **Execution Start:** Tech Lead formalizes task specifications + prompts for sub-agents in `/LLM_Journey/a-docs/communication/conversations/tech-lead-to-<role>.md`.
   - **Template Rule**: Use role-specific handoff templates in `/LLM_Journey/a-docs/communication/conversations/TEMPLATE-tech-lead-to-<role>.md`.
...
- Conversation handoff/report files under `a-docs/communication/conversations/` are **single-CR working artifacts**.
...
To reduce ceremony at CR boundaries, pre-populate the check at the top of the incoming CR content using this structure — fill in the blanks during replacement:
    ...
    - Evidence 1 (plan artifact exists): `a-docs/workflow/plans/[prior CR-ID]-plan.md`
    - Evidence 2 (prior CR closed): `a-docs/workflow/requirements/[prior CR-ID]-[slug].md` status is `Done`
...
- **The Tech Lead writes the Handoff**: This is the final action of the Tech Lead Agent for a specific sub-task. Use the role-specific handoff templates in `/LLM_Journey/a-docs/communication/conversations/TEMPLATE-tech-lead-to-<role>.md`.
- **The "Wait" State**:
...
3. The handoff file location(s) (e.g., `a-docs/communication/conversations/tech-lead-to-frontend.md`)
...
### Code & Git Standards
- All contributions must follow the rules defined in `a-docs/development/contribution.md`.
...
5. **Halt on Blocker/Assumption Invalidation**: If a blocker (missing contract, environmental discrepancy, or logical flaw) is identified:
   ...
   - Report the issue immediately via the `a-docs/communication/coordination/feedback-protocol.md`.
...
8. BA updates requirement status in `/LLM_Journey/a-docs/workflow/requirements/CR-XXX-<slug>.md`.
...
Every ID mentioned in the `$LLM_JOURNEY_LOG` (e.g., `CR-XXX`, `ADR-XXX`) **MUST** have a corresponding artifact in the relevant directory (`$LLM_JOURNEY_WORKFLOW_REQUIREMENTS`, `$LLM_JOURNEY_WORKFLOW_PLANS`, `$LLM_JOURNEY_WORKFLOW_REPORTS`, `$LLM_JOURNEY_GOVERNANCE_DECISIONS`). Do not reference identifiers that do not exist as files.
