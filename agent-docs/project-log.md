...
## Next Priorities
-   [ ] [S] **Live streaming manual validation before production deployment** *(CR-021 TL recommendation)*: E2E tests run in fallback mode only (no `FRONTIER_API_KEY` in test env). Manual browser validation of frontier chat and all three adaptation strategies against a live API key is required before promoting to production.
-   [ ] [S] **Adaptation strategy tabs not disabled during streaming** *(CR-021 TL UX gap)*: `adaptation-chat-tab-*` buttons are not disabled during streaming; clicking a tab mid-stream changes tab state while streaming continues on the prior strategy. Consider a follow-up micro-CR.
