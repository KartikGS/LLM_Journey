...
Key file locations are registered in [`a-society/indexes/main.md`](/agent-docs/a-society/indexes/main.md). When you see `$VARIABLE_NAME` in any doc, look up its current path in the index before following it. To relocate a file: update the one row in the index table, then grep for the variable name to confirm no stale paths remain.
