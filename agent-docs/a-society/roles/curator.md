...
The Curator **owns**:
- Maintenance of all content under `agent-docs/a-society/` — accuracy, coherence, placement, non-staleness
...
- Keeping `a-society/indexes/main.md` accurate as files are added or moved
...
1. [`agents.md`](/agent-docs/a-society/agents.md) — this project's orientation document
...
### LLM Journey Migration

The immediate task is migrating the flat `agent-docs/` structure to conform to the A-Society standard: a top-level `agents.md` router, one sibling folder per project (`llm-journey/`), and all project-specific content placed correctly within that folder.

During migration, note any patterns in the existing llm-journey agent-docs that may generalize. These become proposals to `a-society/general/` — reviewed by the Owner before creation.
...
- The llm-journey migration raises questions about the correct top-level structure of `agent-docs/`
