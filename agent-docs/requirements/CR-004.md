...
- [ ] `pnpm test:e2e` passes consistently across all browsers (Chromium, Firefox, Webkit) when run together.
- [ ] Webkit tests no longer display a "white screen" and correctly render the landing page.
- [ ] Middleware rate limiting does not block legitimate telemetry from local E2E tests.
- [ ] E2E tests can be run at least twice in a row without needing a server restart (idempotency).
